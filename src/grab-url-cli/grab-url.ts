...
import { ArgParser, isFileUrl } from './arg-parser.js';
import { ColorFileDownloader } from './downloader.js';
...
if (__isMain) {
  const argv = new ArgParser()
    ...
    .option('params', {
      ...
      coerce: (arg: string) => {
        if (!arg) return {};
        try { return JSON.parse(arg); } catch { throw new Error(`Invalid JSON in params: ${arg}`); }
      }
    })
    .help()
    .alias('h', 'help')
  ...
  (async () => {
    if (isDownloadMode) {
      ...
      const downloadObjects = urls.map((url, i) => {
        let filename = (i === 0 && outputFile) ? outputFile : downloader.generateFilename(url);
      ...
      try {
        ...
        downloadObjects.forEach(obj => {
          try {
            ...
            statsTable.push([obj.filename, downloader.formatBytes(stats.size), stats.birthtime.toLocaleString()]);
          } catch {
      ...
      try {
        ...
        if (typeof res.data === 'string') {
          outputData = res.data; isTextData = true;
        } else if (Buffer.isBuffer(res.data) || res.data instanceof Uint8Array) {
          outputData = res.data; isTextData = false;
        } else if (res.data instanceof Blob) {
          outputData = Buffer.from(await res.data.arrayBuffer()); isTextData = false;
        } else if (res.data && typeof res.data === 'object') {
          outputData = JSON.stringify(res.data, null, 2); isTextData = true;
        } else {
          outputData = String(res.data); isTextData = true;
        }
        ...
        if (!noSave) {
          ...
          else log(`Binary data received (${outputData.length} bytes). Use --output to save to file.`);
        }
      } catch (error: any) {
