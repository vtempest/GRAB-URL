...
import _rawSpinners from '../icons/cli/spinners.js';

// spinners.js exports: { spinnerName: string[], ... }  (flat arrays of frames)
// Normalise to a consistent { name: { frames: string[] } } shape
type SpinnerData = Record<string, string[] | { frames: string[] }>;
const spinnerData: SpinnerData = _rawSpinners as SpinnerData;

function getFrames(entry: string[] | { frames: string[] }): string[] {
...
export function getSpinnerFrames(spinnerType: string): string[] {
    const entry = spinnerData[spinnerType];
    if (entry) return getFrames(entry);
    const dots = spinnerData['dots'];
    return dots ? getFrames(dots) : ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];
}
